...
From a practical business perspective, this example also shows why A/B testing should not only focus on whether a result is statistically significant. The size of the effect, the uncertainty around the estimate, and the testing procedure all matter when deciding whether one version should replace another. For example, even if CTA A has a higher observed sign-up rate, I would still want to check whether the improvement is large enough to matter in practice and whether the experiment was run without repeated peeking.
